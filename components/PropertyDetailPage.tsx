...
export default function PropertyDetailPage({ propertyId }: { propertyId: string }) {
  ...
  useEffect(() => {
    ...
    console.log('PropertyDetail: パラメータID:', propertyId)
    console.log('PropertyDetail: Supabase設定:', {
      url: process.env.NEXT_PUBLIC_SUPABASE_URL,
      hasAnonKey: !!process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY
    })
    if (propertyId.startsWith('suumo-')) {
      fetchSuumoProperty(propertyId)
      return
    }
  ...
  const fetchSuumoProperty = async (id: string) => {
    try {
      ...
      const normalized: Property = {
        ...
        station: item.transportation?.[0]?.station || '',
        route: item.transportation?.[0]?.line || '',
        walking_time: item.transportation?.[0]?.walk_time
          ? Number(String(item.transportation[0].walk_time).replace(/[^\d]/g, '')) || undefined
          : undefined,
        ...
        overview: item.overview && typeof item.overview === 'object' ? item.overview : {},
        equipment_notes: Array.isArray(item.equipment_notes) ? item.equipment_notes : [],
        surroundings: Array.isArray(item.surroundings) ? item.surroundings : [],
          ...
          <div className="lg:col-span-2">
            ...
            <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {property.traffic && (!property.transportation || property.transportation.length === 0) && (
                  <div className="col-span-2">
                    <span className="text-gray-600">交通</span>
                    <p className="mt-1 text-sm text-gray-700">{property.traffic}</p>
                  </div>
                )}
                {property.layout && (
                  <div>
            ...
            {property.equipment_notes && property.equipment_notes.length > 0 && (
              <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
                <h2 className="text-xl font-bold mb-4">設備・構造</h2>
                <ul className="list-disc pl-6 space-y-2 text-gray-700">
                  {property.equipment_notes.map((note, index) => (
            ...
            {property.overview && Object.keys(property.overview).length > 0 && (
              <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
                <h2 className="text-xl font-bold mb-4">物件概要</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-700">
                  {Object.entries(property.overview).map(([label, value]) => (
              ...
              <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
                ...
                <ul className="space-y-2 text-sm text-gray-700">
                  {property.surroundings.map((item, index) => (
                    <li key={`${item.name}-${index}`} className="flex flex-wrap gap-2">
                      <span className="font-medium">{item.category}</span>
                      <span>{item.name}</span>
                      {item.distance && <span>{item.distance}</span>}
                      {item.walk_time && <span>徒歩{item.walk_time}</span>}
                    </li>
                  ))}
                </ul>
              </div>
