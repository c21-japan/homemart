...
export default function AreaSearch() {
  ...
  const handleAreaClick = (areaName: string) => {
    // ここで物件一覧をフィルタリング
    console.log(`${areaName}の物件を検索`)
    // 実際の検索処理は後で実装
    window.location.href = `/properties?area=${encodeURIComponent(areaName)}`
  }
    ...
    <div className="w-full max-w-7xl mx-auto p-4 bg-white rounded-lg shadow-sm">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
        {areaData[selectedPrefecture].map((area) => (
          <button
            key={area.name}
            onClick={() => area.count > 0 && handleAreaClick(area.name)}
